...
storybook_nsl theme uses [Webpack](https://webpack.js.org) to compile and
bundle SASS and JS.
...
Go to the root of storybook_nsl theme and run the following commands:
`nvm use`
...
`drush en storybook_nsl -y`
`drush config-set system.theme default storybook_nsl -y`
